    ...
    */
    ...
    inline uint32_t vudaGetFamilyQueueIndex(const std::vector<vk::QueueFamilyProperties>& queueFamilyProperties, const vk::QueueFlags queueFlags)
    {
        struct FamilyQueueCandidate {
            FamilyQueueCandidate(const vk::QueueFlags inc, const vk::QueueFlags exc)
                : m_include(inc)
                , m_exclude(exc)
            {
            }
        ...
        for (const FamilyQueueCandidate& fqc : candidates) {
            ...
            if ((queueFlags & fqc.m_include) == queueFlags) {
                size_t queueFamilyIndex = std::distance(queueFamilyProperties.begin(), std::find_if(queueFamilyProperties.begin(), queueFamilyProperties.end(), [fqc](vk::QueueFamilyProperties const& qfp) {
