...
enum enumvudaError { vudaSuccess = 0,
    vudaErrorInvalidDevice = 101 };
enum memcpyKind { memcpyHostToHost = 0,
    memcpyHostToDevice = 1,
    memcpyDeviceToHost = 2,
    memcpyDeviceToDevice = 3 /*, memcpyDefault = 4*/ };
enum enumHostAlloc { hostAllocDefault = 0,
    hostAllocPortable = 1,
    hostAllocMapped = 2,
    hostAllocWriteCombined = 3 };
...
struct dim3 {
    ...
    dim3(const uint32_t x)
        : x(x)
        , y(1)
        , z(1)
    {
    }
    dim3(const uint32_t x, const uint32_t y)
        : x(x)
        , y(y)
        , z(1)
    {
    }
    dim3(const uint32_t x, const uint32_t y, const uint32_t z)
        : x(x)
        , y(y)
        , z(z)
    {
